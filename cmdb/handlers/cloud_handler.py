...
def get_job_func(cloud_name, account_id):
    def job_func():
        sync_func = cloud_loader.get_sync_function(cloud_name)
        if sync_func:
            try:
                sync_func(account_id=account_id)
...
def add_cloud_jobs():
    ...
    for item in resp:
        ...
        job_func = get_job_func(cloud_name, account_id)
        if not job_func: continue
        scheduler.add_job(
            job_func, 'interval', minutes=item.get('interval', 30),
